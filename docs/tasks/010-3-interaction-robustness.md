...
## 折叠、计时与报告修复及实际操作验收（2026-09-26）

用户反馈上一轮过程可见但无法折叠、计时落到回复底部、报告不易看到。
此前只验证默认展开和 DOM 内容，未充分验证点击后正文隐藏，验收不足。

- 根因：CollapsibleContent 的 asChild 直接使用 Markdown/Streamdown，未完整转发 hidden 等 DOM 属性。
  改为原生 button 和原生正文容器承接折叠及 aria 属性；正文最高 24rem，可独立滚动。
- 计时显示在“解释过程”标题旁，业务过程回复底部移除重复耗时；原 usage 和复制功能保留。
- 报告独立于过程折叠容器，补充“解释报告”标题；无报告的安全终态文案标为“解释结果”。
- 用户最新井在 W09 REVIEW_REQUIRED，原聊天只显示复核提示，已有诊断报告只在面板显示。
  本版如有诊断报告，Streamer 读取同一 execution_id 并展示“人工复核诊断报告”，明确未通过验证。
  不改变 Workflow、复核判定、公司工具或持久化事实，也不改写旧持久消息。
- 无新增文件。修改 ASMessageBubble.tsx、zh/en.json、execution_stream.py、test_demo_web_upload.py，
  AGENTS.md 及两份设计/报告文档。AGENTS.md 增加用户要求的页面实际操作验收规则。

真实页面验收在 admin 的独立会话“页面回归测试解释”中完成：

| 实际操作 | 结果 |
| --- | --- |
| 上传 WELL_MOCK_001.json 并解释 | W01–W10 成功，顶部过程计时 10s，过程外显示完整报告 |
| 点击过程折叠与展开 | 正文隐藏/恢复，折叠后“解释报告”仍可见；原生容器 hidden=true、高度为 0 |
| POR=0.17 参数修改 | 模型此次先要求澄清，补答“孔隙度”后仅生成 #2，W01–W03 复用，报告孔隙度 17.00% |
| 修改后的过程折叠 | 7s 计时仍在标题旁，过程折叠，第二版报告保留 |
| 上传独立的证据不足测试井 | W09 进入复核，不执行 W10，9s 过程及人工复核诊断报告显示 |
| 折叠复核过程 | 过程关闭，诊断报告可见，不显示成功结论 |
| 刷新页面 | 三段过程默认展开、10s/7s/9s 标题计时、两份成功报告及诊断报告均恢复；底部没有重复耗时 |

启动后首次提交跨过后端重启造成原 SSE 连接中断；刷新重连后已恢复，并继续上述页面验收。
测试期间部分自动化定位读取超时，均重新获取页面状态后检查实际展开/隐藏结果，未把超时写成通过。

- 前端：5 tests passed；build 通过（现有 chunk 大小提示）；修改组件 eslint 通过。
- 后端相关上传/流式测试：18 passed，新增公司 Mock 的 REVIEW_REQUIRED 诊断报告回归。
- 全量真实 PG/Redis：370 passed、2 skipped、1 warning；跳过项与前文相同。
- 修改 Python 文件 ruff check / 生产文件 format check、git diff --check 通过；后台无 Traceback/ERROR。
- 仍有前文记录的既有前端附件 key 提示；旧复核回复未重写，可从任务面板或报告查询读取诊断报告。
- 无新增下一阶段依赖。No Architecture Issue found.
