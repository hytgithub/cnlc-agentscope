...
async def test_commands_reuse_planner_and_preserve_full_rerun_overrides(data_dir):
    ...
    for changes, reused in [
        ...
        assert result.execution_status == ExecutionStatus.QUEUED
        result = await session.wait_for_completion(initial.task_id, result.execution_id)
    ...
    full = await session.wait_for_completion(initial.task_id, full.execution_id)
...
async def test_status_and_report_selectors_use_repository_and_check_binding(data_dir):
    ...
    second = await session.wait_for_completion(first.task_id, second.execution_id)
    ...
    assert explicit.report_markdown == await session.repository.get_execution_report(
        first.execution_id
    )
    other = await session.run("WELL_MOCK_001")
    await session.wait_for_completion(other.task_id, other.execution_id)
    ...
    assert status.execution_status == saved.status
    ...
    assert await session.repository.claim_execution(
        failed.workflow_execution_id, "test-worker", utc_now() + timedelta(minutes=1)
    )
    await session.repository.finish_execution(
    ...
    assert actual.execution_status == ExecutionStatus.FAILED
    await session.dispatcher.shutdown()
